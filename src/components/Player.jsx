	...
	return (
		<div className={`
			...
			<div className="flex flex-col">
				...
				<span className="text-xs" >{artists.join(", ")}</span>
			</div>
		</div>
	);
}
...
const SongControl = ({ audio }) => {
	...
	return (
		<div className="flex gap-x-5 text-xs mb-3">
			<span className="text-white/60 w-12 text-right">{formatTime(currentTime)}</span>
			<Slider
				defaultValue={[0]}
				value={[currentTime]}
			...
			/>
			<span className="text-white/60 w-12">{duration ? formatTime(duration) : "0:00"}</span>
		</div>
	);
}
	...
	return (
		<div className="flex justify-center gap-x-2">
			<button onClick={handleClickVolumeIcon}>
				{isVolumeSilenced ? <VolumeSilence /> : <VolumeUp />}
			</button>
			<Slider
				defaultValue={[100]}
				max={100}
		...
		<div className="flex flex-row justify-between w-full px-1 z-50">
			...
			<div className="grid place-content-center gap-4 flex-1">
				<div className="flex justify-center flex-col items-center pt-2">
					<button className="bg-white rounded-full p-2 mb-2" onClick={handleClick}>
